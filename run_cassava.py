...
def get_base_config():
    ...
    dt_cfg_path = str(_ROOT / "dataset_dependent_configs/cassava_config.yaml")
    with open(dt_cfg_path) as f:
        config = config_update(config, yaml.full_load(f))
        
    # Set global defaults for Cassava
...
def run_fold(model_name, fold_idx, config):
    ...
    config["phase"] = "train"
    seed_torch(config.get("seed", 42))  # Cassava seed is 42
    ...
    ckpt_dir = os.path.join(
        _ROOT, "Lich_su_train", "cassava", model_name, f"fold_{fold_idx}"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Cassava MIL Experiments")
    parser.add_argument(
        "--mode",
        type=str,
        default="run_all",
        choices=["run_all"],
        help="run_all: run full 5-fold CV for 5 models",
    )
    ...
    report_path = os.path.join(output_report_dir, "cassava_results.md")
    
    with open(report_path, "w") as f:
        f.write("# Cassava Dataset Benchmark Results (Sanity Check)\n\n")
        f.write("Experiments conducted on a subset of Cassava leaf images (5 classes: Bacterial Blight, Brown Streak Disease, Green Mottle, Mosaic Disease, Healthy).\n\n")
